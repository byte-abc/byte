...
vi.mock('../', async () => {
  ...
  return {
    ...actual,
    createAliasRegexp: vi.fn(actual.createAliasRegexp),
    viteAliasCustomResolver: vi.fn(actual.viteAliasCustomResolver),
  }
})
...
describe('viteAliasCustomResolver', () => {
  ...
  it('should return alias with root c:\\xx\\xx\\', () => {
    ...
    expect(resolve('/foo/bar', testPath0)).toBe(
      '/Users/app/packages/vue-components/src/headless/form/__stories__/foo/bar',
    )
    expect(resolve('foo/bar', testPath1)).toBe(
      '/Users/app/apps/vue-components/src/headless/form/__stories__/foo/bar',
    )
    expect(resolve('foo/bar', undefined)).toBe('foo/bar')
  })
})
